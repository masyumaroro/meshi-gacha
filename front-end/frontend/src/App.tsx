...
export default function App() {
  ...
  const spinGacha = async () => {
    ...
    try {
      // Java APIへのリクエスト
      const response = await fetch(`http://localhost:8080/api/foods/gacha?${params.toString()}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-yellow-50 to-pink-50 p-6 font-sans text-gray-800">
      <div className="max-w-2xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          ...
          <p className="text-gray-500 font-medium">今日のご飯、迷ったらJavaに聞け！</p>
        </motion.div>
